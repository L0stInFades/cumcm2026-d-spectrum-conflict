...
MEMBER_DESCRIPTIONS = (
    ...
    (r"^pipelines/d/validators\.py$", "与求解器分离的独立校验器（按定义展开单元集合求交）"),
    (r"^pipelines/d/detect\.py$", "问题 1：成对检测与按频段分桶的扫描线检测"),
    (r"^pipelines/d/resolve\.py$", "问题 2/4：动作集合、单元格模型、词典序与加权求解"),
    (r"^pipelines/d/pack\.py$", "问题 3：候选放置、集合装填模型与重排构造"),
    (r"^pipelines/d/report\.py$", "论文图与表的生成"),
    (r"^pipelines/d/synth\.py$", "微型/合成实例与穷举参照解"),
    (r"^pipelines/d/stages\.py$", "本题各阶段的编排"),
    (r"^pipelines/", "本题建模与求解程序"),
    (r"^tools/run_local\.py$", "不依赖云端的本地驱动脚本"),
    (r"^tools/", "本地控制面：提交云端任务、下载、校验散列"),
    (r"^tests/", "单元 / 性质 / 集成测试"),
    (r"^configs/hints/", "词典序求解的热启动现任解（含来源 run 与目标向量）"),
    ...
    (r"solver_report\.json$", "求解器逐级证据（值、证明下界、状态、用时）"),

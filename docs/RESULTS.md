...
新增 C 类计划 `QthreeNewPlans` = **109**，CP-SAT 状态 OPTIMAL、上界 109、gap = 0（`QthreeGap` = 0，`QthreeOptimal` = 通过）。候选放置位置 1344，AtMostOne 约束 4378，贪心下界 103。T_end = 643。

**备选解释 B（既有计划可任意平移，MDR-0010）**：首次适配重排后候选位置增至 34761、约束 43488，贪心即可放入 **557** 个新增 C 类计划——远高于解释 A 的 109。这说明两种解释给出的答案相差一个数量级，论文必须明确采用哪一种并给出另一种的区间。（CP-SAT 在 300 s 内的最终值与面积上界见 `tab_q3_summary` 与下表。）

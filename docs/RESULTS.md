...
5. 问题 3 假定新增计划参数与既有 C 类完全相同，且既有计划不动（MDR-0005）。其结果依赖于所选的问题 2 方案：撤销更少的方案留下更多被占用的单元，可容纳的新增计划相应更少；论文应说明报告的数量是"在本文问题 2 方案之上"的最大值，而非跨所有无冲突方案的最大值。
...
**论文可用的核心结论**：撤销数量对**频段平移上限**极其敏感——上限由题目的 10Δf 放宽到 15Δf 时，撤销数从 12 直接降到 **0**（即不必撤销任何计划）；反之收紧到 5Δf 时升至 42–59。时间平移上限的影响较小（(10,2) 与 (10,5) 之间差 12 个撤销，而 (10,5) 与 (15,5) 之间差 12 个撤销却只放宽了 5 个频段）。这说明冲突主要是**频域拥塞**而非时域拥塞，与问题 1 的统计一致（BC 类冲突 181 对占 61%，而 B 类频宽 15 最大）。若能把最大频移放宽到 15Δf，代价仅是多调整 1 个计划、总幅度由 75.0 升到 98.4，却可完全避免撤销——这是一条有实际价值的管理建议。
...
| 论文门禁 | `qa` 16 项检查全部通过，身份信息黑名单命中 0，观察名单命中 0 |
...
| `fig_q2_resolution` | 第 5 节（问题 2）主图 | 问题 2 的冲突消解方案。(a) 消解前：按所采取的动作着色（灰 = 保持、蓝 = 频段平移、橙 = 时间平移、斜线填充 = 撤销），黑边为发生冲突的使用次；(b) 消解后：141 个存留计划在时频平面上已无任何冲突，箭头指示每个被平移计划的移动方向。 |

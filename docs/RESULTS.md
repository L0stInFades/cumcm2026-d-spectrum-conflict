...
9. 问题 4 的解把最晚结束时刻由 643 延长到 718（+75Δt，见 MDR-0006 补充）。题目只在问题 3 中提出"不增加时频资源"，未对问题 4 设此约束，故本文不对结束时刻设限；但若评阅者按"资源总量不变"理解，该解会被视为多占用了时间资源。加入 `p.end <= 643` 硬约束重解的版本本轮未做，列为局限。
...
**论文可以并且只可以声称**：撤销数量 (0, 0, 12) 已被 CP-SAT 证明为该词典序前三级的全局最优——即"在不撤销任何 A、B 类计划的前提下，必须撤销的 C 类计划数最少为 12"，这是题目首要目标上的**最优性证明**；由该级的 OPTIMAL 状态直接得到推论：**在不撤销任何 A、B 类计划的前提下，"一个都不撤销"是不可行的**（因为该级的最小值 12 > 0），这一推论完全来自 `tab_q2_solver` 的"撤销 C = 12 / 下界 12 / OPTIMAL"一行，无需额外实验。调整各级与幅度级只报告"值 / 证明下界 / 状态"，不得声称最优。

**关于调整各级的稳健性（诊断复核，非本 run 的阶段产物）**：为判断 16 这个值是预算不足还是接近真实最优，用 `exec` 在云端以同一模型、同一固定等式（撤销 = 0/0/12）、同一现任解起点，把"调整 A"一级的时限由 300 s 提高到 **900 s**（3 倍预算）重解，结果仍为 **16**，证明下界仅由 7 提高到 8（状态 FEASIBLE）。结论：报告的 16 对预算不敏感，该级真实最优值落在 **[8, 16]**；论文可据此说明"更长预算未能改进现任解"，但**不得**把 900 s 的数字当作结果引用（它不在 run 的阶段产物中，没有 `\val` 宏）。若要将其正式化，应把该时限写进 `level_time_limits` 参数并重跑 `resolve` 及其下游阶段。
...
**这是论文中最重要的一张对照表**：方案 T/S 把撤销总数从 12 降到 8，但**代价是撤销 2 个 A 类和 6 个 B 类计划**；题目明确要求"A 类优先级最高、B 类其次、C 类最低，高优先级用频装备的用频计划应尽量保持"，故本文采用 P。论文应直接引用这一对照说明 P 的选择不是任意的：P 是**唯一在问题 2 与问题 4 中都不撤销任何 A、B 类计划**的方案（W 在问题 4 中恰好也做到 0/0/12，但在问题 2 中撤销了 5 个 B 类；T 与 S 在两题中都撤销 A 或 B 类）。（备选方案每级时限仅 30 s，其向量只作定性对照，不作最优性声明。）

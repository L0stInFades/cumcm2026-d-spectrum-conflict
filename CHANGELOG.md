...
按三位独立审稿人（数学与数值严谨性 / 竞赛合规与可复现性 / 写作与表达）的 46 条意见修订，
逐条处理说明见 `docs/REVIEW_RESPONSE.md`。运行 `20260912-212914-2b9c081`。

### Added
- `coldstart` 阶段：不加载现任解重解问题 2，回答"仓库里存的是搜索的输出还是输入"。
- 条件最优性（MDR-0012）：固定撤销集合后逐级最小化，并在其解词典序更优时采用之。
- 问题 4 的资源守恒硬约束（MDR-0011）与不设限变体对照；四个布局上的装填对照（MDR-0014）。
- `tools/run_local.py`：不依赖 Modal 账号的本地驱动；`REPRODUCE.md` 给出云端与本地两条路径。
- 冲突图最大独立集下界、单位增量弹性宏、逐点灵敏度宏。

### Changed
- NP-难性改述为轴平行矩形相交图的受限族（Fowler–Paterson–Tanimoto 1981）。
- 对未证明层级的下界明确限定适用范围；热启动是否改进如实报告。
- 正文压缩至 30 页（行距 1.12、图宽下调、两张表移入附录）。

### Fixed
- 命题"问题 4 不劣于问题 2"在加入资源守恒约束后不再无条件成立，已改述。
- `BenchScaleMax*` 取最后一个规模而非最大值；图例中仅以黑边表示的条目不可见；表号断号。

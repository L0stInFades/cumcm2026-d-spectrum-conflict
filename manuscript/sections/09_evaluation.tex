...
\begin{enumerate}[leftmargin=1.8em,itemsep=0.25em]
  ...
  \item \textbf{结果可复现}。全部计算在同一次云端运行中完成，每个数字都由阶段产物自动注入论文，
        每个阶段记录代码版本、参数与输入输出的 SHA-256（附录~\ref{app:repro}）。
\end{enumerate}
...
\begin{enumerate}[leftmargin=1.8em,itemsep=0.25em]
  ...
  \item \textbf{问题 3 的答案依赖于具体的问题 2 方案}。定理~\ref{thm:q3} 的 $\valQthreeNewPlans$ 是
        “在本文这一个无冲突方案之上”的最大值。由引理~\ref{lem:tiling}，可装填数量主要取决于既有布局与相位划分的
        \emph{对齐程度}，而非自由单元的\emph{总数}——因此不同的问题 2 最优解会给出不同的答案。
        若要回答“跨所有无冲突方案的最大值”，需把问题 2 与问题 3 合并为一个联合优化，规模将显著增大。
  \item \textbf{问题 4 的方案延长了时间范围}。间隔变长把最后一次使用推后，最晚结束时刻由 $\valQtwoHorizonAfter$
        增至 $\valQfourHorizonAfter$（第~\ref{sec:q4horizon} 节）。题目未对问题 4 提出“不增加时频资源”的要求，
        但若按此理解，应加入硬约束 $e_i\le\valQtwoHorizonAfter$ 重解；该变体本文未做。
...
\textbf{其二}，允许 C 类装备微调用频间隔是低成本的松弛——它把撤销数从 $\valQtwoCancelTotal$ 降到 $\valQfourCancelTotal$
（表~\ref{tab:q4compare}），但需注意其代价是时间范围延长 $\valQfourHorizonDelta\Delta t$；

...
\noindent\textbf{本节回答}：本文的方法在什么范围内可信，在什么地方会失效？
概括地说，优点在于“无冲突”的\emph{充要}编码与与求解器分离的独立校验，使每个结果都可被逐位复核；
主要局限在于最优性只在撤销层（及问题 3）被证明、调整层只有值与下界、
以及若干题意读法（目标的优先级形式、问题 3 的两种解释、问题 4 是否受资源守恒约束）本文只能并列给出而无法判定。

...
都在云端（Modal）执行，本地只负责编辑文本、提交任务、下载产物并核对 SHA-256。复现步骤如下：

\begin{enumerate}[leftmargin=1.8em,itemsep=0.15em]
  \item 安装 Modal 客户端并完成鉴权后，在仓库根目录执行 \nolinkurl{python3 tools/cli.py provision}
        验证云端工具链（\TeX{} Live、中文字体、CP-SAT、HiGHS）。
  \item 数据接入与契约校验：\nolinkurl{python3 tools/cli.py run ingest,validate,detect --new-run --size small}。
  \item 两个重阶段分离运行（各约 $40$ 分钟）：\nolinkurl{run resolve --size large --spawn} 与
        \nolinkurl{run resolve_interval --size large --spawn}，随后 \nolinkurl{wait <ID>} 或轮询 \nolinkurl{status}。
  \item 其余科学阶段：\nolinkurl{run pack,bench,sensitivity --size large}，再 \nolinkurl{run compare,results,figures,tables}。
  \item 门禁与论文：\nolinkurl{run lint,test} 与 \nolinkurl{run paper,qa,package,release}。
  \item 下载与核对：\nolinkurl{download --stage <name>} 会逐文件核对 SHA-256；
        \nolinkurl{release --version <tag>} 生成交付目录与发布清单。
\end{enumerate}

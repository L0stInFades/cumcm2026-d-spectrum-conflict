...
{\footnotesize\input{generated/tables/tab_validation.tex}}
\end{table}
...
（$\valBenchScaleDetectorsAgree$，附录表~\ref{tab:benchscaling}）。加上第~\ref{sec:validation} 节的单元集合校验器，
...
\begin{enumerate}[leftmargin=1.8em,itemsep=0.2em]
  \item \textbf{撤销数量对频段平移上限极其敏感。}把 $\Phi$ 由 $10$ 放宽到 $15$，撤销数由 $\valSensBaseCancelTotal$
        直接降到 $\valSensMinCancelTotal$——\emph{一个计划都不必撤销}；反之收紧到 $\Phi=5$，撤销数升至
        $\valSensMaxCancelTotal$。这再次印证第~\ref{sec:q1} 节的判断：
        \textbf{撤销源于局部拥塞与幅度上限，而非资源总量不足}。
  \item \textbf{频域比时域更“贵”。}把两个上限分别放宽，可以定量比较二者的作用：
        $\Phi$ 由 $5$ 增至 $10$（$\mathcal{T}$ 固定）使撤销数减少约三十个；
        而 $\mathcal{T}$ 由 $2$ 增至 $5$（$\Phi$ 固定）只减少约十余个。
        \textbf{频域放宽的效果约为时域的两倍}——这与问题 1 中“B--C 类冲突占六成以上、而 B 类频宽最大”的统计一致。
...
\paperfigure[0.94\linewidth]{fig_sensitivity}{消解方案对最大平移幅度的灵敏度。(a) 被调整（按类别堆叠）与被撤销（黑）的计划数；
(b) 归一化总调整幅度。频段平移上限由 $10\Delta f$ 放宽到 $15\Delta f$ 时撤销数降为 $\valSensMinCancelTotal$，
收紧到 $5\Delta f$ 时升至 $\valSensMaxCancelTotal$，说明冲突主要源于频域拥塞。}{fig:sens}

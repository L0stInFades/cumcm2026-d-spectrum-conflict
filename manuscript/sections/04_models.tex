...
且该最大度由 $\valQoneTopDegreeCount$ 个装备同时达到（$\valQoneTopDegreeIds$，附录表~\ref{tab:q1deg}）。这提示“局部让路”的空间是存在的：
...
为把这一点做实，本文另设一个\emph{冷启动}阶段：同一模型、同一目标、同一时限，但\textbf{不加载任何现任解}，
从零开始求解问题 2。它得到的向量为 $\valColdVector$，与热启动结果 $\valColdWarmVector$ 的关系是
“\valColdRelation{}”（撤销总数相差 $\valColdCancelDelta$、调整总数相差 $\valColdAdjustDelta$）。
该阶段不参与任何交付工作簿的生成，只回答一个方法层面的问题：仓库中存放的现任解，是搜索的\emph{输出}还是\emph{输入}。

答案必须说清楚：\textbf{在本文的时限下，它是输入}。单次冷启动没有达到 $\valQtwoVector$，
因此“任何人重跑一次就能得到同一答案”这句话只有在\emph{加载仓库中的现任解}时才成立——
而该文件本身是更早若干次运行累积的最好可行解，其来源 run 与 commit 记录在文件内。
这并不削弱本文的任何结论，理由有三：
（i）$\valQtwoVector$ 是一个\emph{已被独立校验器复核过的真实可行解}（零剩余冲突、全部动作合法），
可行性与它从哪里来无关；
（ii）撤销三级的状态为 \textsf{OPTIMAL}，由命题~\ref{prop:cut}，该结论与起点无关；
（iii）未闭合的各级本文本就只报“值 / 证明下界 / 状态”，冷启动的对照恰好量化了起点对这些层级的影响有多大。
把这一点如实写出，比声称“完全可从零复现”更接近事实。
...
\paragraph{受限再解同时是最后一次改进。}受限问题的可行集是完整问题可行集的\emph{子集}，
因此受限再解找到的任何点在完整问题中同样可行。若它在词典序上\emph{严格优于}主搜索报告的现任解，
继续报告后者就等于明知有更优解而不报。本文因此在受限再解之后加一步判定：更优即采用，
并重算表~\ref{tab:q2table1} 的分类统计、全部幅度统计与独立校验，同时登记这一事件。
本问是否发生了采用：\valQtwoCondAdopted{}；问题 4 见第~\ref{sec:q4} 节（\valQfourCondAdopted{}）。

采用发生时，两张表的分工需要说清楚：表~\ref{tab:q2solver} 是\emph{无约束逐级搜索}的轨迹
（各级的值、证明下界与状态，以及据此得到的界），而表~\ref{tab:q2conditional} 给出的才是\emph{最终报告解}
在调整三级与幅度级上的取值——并且它们在固定撤销集合这一条件下全部已证明最优。
两张表的撤销三级一致（采用不改变撤销集合）。
...
\paragraph{优先级条款的第二重代价。}表~\ref{tab:q3layouts} 还给出一个题目本身没有问、但对用频管理有意义的结论：
四个布局中\emph{最不利于后续新增}的恰是方案 $\valQthreeLayoutMinScheme$（可再安排 $\valQthreeLayoutMin$ 个，
撤销 $\valQthreeLayoutMinCancelled$ 个），而最有利的是方案 $\valQthreeLayoutMaxScheme$
（$\valQthreeLayoutMax$ 个，撤销 $\valQthreeLayoutMaxCancelled$ 个）；
本文主方案 P 是四者中唯一不撤销任何 A、B 类计划的方案，它是否恰为最不利者：\valQthreePrimaryIsWorstHost{}。
若该判定为“通过”，则坚持“高优先级计划尽量保持”这一条款的代价是\emph{双重}的：
当期要多撤销若干 C 类计划，而且留给后续新增计划的空间也更小。
这一点在题目的四个问题里不会出现，但在实际用频规划中值得纳入权衡——
本文只报告这一观察，不据此建议放弃优先级条款（优先级本身是题目给定的硬性要求）。

...
\paperfigure[0.78\linewidth]{fig_q1_gantt}{附件 1 中 $\valQonePlans$ 个用频计划在时频平面上的甘特图。
...
\paperfigure[0.78\linewidth]{fig_q1_structure}{冲突的结构。(a) 按装备类别对统计的冲突对数矩阵（共 $\valQoneConflictPairs$ 对，
...
\noindent\textbf{本节回答}：如何以最小代价消除第~\ref{sec:q1} 节检出的全部冲突？结论是保留 $\valQtwoKeepTotal$、
调整 $\valQtwoAdjustTotal$、撤销 $\valQtwoCancelTotal$，且撤销的全部是 C 类；撤销三级已闭合对偶间隙（定理~\ref{thm:q2opt}），
调整各级只报“值 / 证明下界 / 状态”。
...
这并不削弱结论：报告解是经独立校验器复核过的真实可行解，可行性与它从哪里来无关；撤销三级状态为
\textsf{OPTIMAL}，由命题~\ref{prop:cut} 与起点无关；未闭合的各级本就只报“值 / 证明下界 / 状态”，
冷启动的对照恰好量化了起点的影响。把这一点写出，比声称“完全可从零复现”更接近事实。
...
\paperfigure[0.72\linewidth]{fig_q2_resolution}{问题 2 的冲突消解方案。(a) 消解前：按所采取的动作着色
...
采用发生时两张表的分工是：表~\ref{tab:q2solver} 为无约束逐级搜索的轨迹（值、证明下界与状态），
表~\ref{tab:q2conditional} 才是最终报告解在调整三级与幅度级上的取值；采用不改变撤销集合，两表的撤销三级一致。
...
\paperfigure[0.82\linewidth]{fig_q2_schemes}{四种目标方案的对照。(a) 各方案下被调整的计划数（按类别堆叠）；
...
结论是 $\valQthreeNewPlans$ 个，且 CP-SAT 对偶界、HiGHS 线性松弛界与 MILP 界三者同时等于该值，即它是\emph{已证明的最大值}。
...
\paperfigure[0.76\linewidth]{fig_q3_layout}{在问题 2 的无冲突方案（灰）之上最多可再安排 $\valQthreeNewPlans$ 个 C 类用频计划（绿）。
...
\paragraph{优先级条款的第二重代价。}表~\ref{tab:q3layouts} 还给出一个题目未问、但对用频管理有意义的结论：
四个布局中最不利于后续新增的是方案 $\valQthreeLayoutMinScheme$（可再安排 $\valQthreeLayoutMin$ 个、撤销 $\valQthreeLayoutMinCancelled$ 个），
最有利的是方案 $\valQthreeLayoutMaxScheme$（$\valQthreeLayoutMax$ 个、撤销 $\valQthreeLayoutMaxCancelled$ 个）；
主方案 P 是唯一不撤销 A、B 类的方案，它是否恰为最不利者：\valQthreePrimaryIsWorstHost{}。
若为“通过”，则坚持优先级条款的代价是双重的：当期多撤销若干 C 类计划，留给后续新增的空间也更小。
...
\paperfigure[0.72\linewidth]{fig_q3_repack}{备选解释 B 下的布局：若允许既有计划不受平移幅度限制地重新放置
...
结论是在不增加时频资源（$e_i\le\valQfourHorizonCap$）的前提下撤销数由 $\valQtwoCancelTotal$ 降到 $\valQfourCancelTotal$，
代价是多调整 $\valQfourAdjustDelta$ 个计划，A、B 两类仍一个未撤。
...
\paperfigure[0.72\linewidth]{fig_q4_resolution}{问题 4 的冲突消解方案，图例与图~\ref{fig:q2res} 相同，
另增“间隔调整”一类（$\valQfourGapChanged$ 个 C 类计划改变了使用间隔）。}{fig:q4res}

\paperfigure[0.82\linewidth]{fig_q4_comparison}{问题 4 与问题 2 的对照。(a) 各类被调整的计划数；

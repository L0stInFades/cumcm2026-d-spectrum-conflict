...
$B=100$ & 频段总数，编号 $0,\dots,B-1$ & $\Delta f$ \\
...
$I_i^{(k)}=[\,s_i+k\pi_i,\ s_i+d_i+k\pi_i)$ & 计划 $i$ 第 $k$ 次使用的时间区间，$\pi_i=d_i+g_i$ 为周期 & $\Delta t$ \\
...
$\mathcal{C}$ & 时频单元全集 $\{0,\dots,B-1\}\times\{0,\dots,T_{\mathrm{end}}-1\}$ & 单元 \\
...
“单计划单元数”为 $w\,d\,n$。}
\label{tab:data}
\input{generated/tables/tab_data.tex}
\end{table}

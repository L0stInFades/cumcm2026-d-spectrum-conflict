...
\begin{itemize}[leftmargin=1.6em,itemsep=0.15em]
  \item \textbf{问题 1} 是判定 $\binom{150}{2}$ 对单元集合是否相交。直接求交代价高，但由于 $U(P)$ 是
  ...
  \item \textbf{问题 2、4} 是：每个计划从一个有限动作集合中\emph{恰选一个}动作（保持、频移、时移、\,问题 4 另加改间隔、撤销），
        ...
        这是一个\emph{列表着色 / 带选项的装填}型问题，其特例（只许保持或撤销）正是冲突图的最大独立集，故为 NP-难。
  \item \textbf{问题 3} 是：在问题 2 方案留下的自由单元中，放入尽可能多的同形状"砖块"（C 类计划的单元集合），
        这是二维\emph{集合装填}（set packing）问题。
\end{itemize}
...
图~\ref{fig:roadmap} 给出本文的技术路线。全部计算在同一次云端运行中完成，每个阶段的输入输出、参数、
代码版本与 SHA-256 散列都写入清单文件，论文中的每个数字都由该次运行的阶段产物自动注入（附录~\ref{app:repro}）。
...
\node[model] (r2c) at (8.2,-2.9)  {\textbf{问题 4}\\C 类增加"改间隔"动作};
...
\node[data]  (r3d) at (12.3,-5.8) {\nolinkurl{result1--4.xlsx}\\论文图、表、数字};
